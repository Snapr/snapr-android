...
{	
	...
	public WebResourceResponse shouldInterceptRequest(WebView view, String url)
	{
		...
		{
			if (Global.LOG_MODE) Global.log("WebViewClientEx.shouldInterceptRequest: Processing URL " + url);
			
			String cacheRootUrl = ((WebViewEx)view).getCacheRootUrl();
			
			if (Global.LOG_MODE) Global.log("WebViewClientEx.shouldInterceptRequest: cacheRootUrl is " + cacheRootUrl);
			
			if (needsCacheCopy(url, cacheRootUrl))
			{
				String sourceFileName = getSourceFileNameFromCacheUrl(url);
				String destFileName = getDestFileNameFromCacheUrl(url);
				if (Global.LOG_MODE) Global.log("WebViewClientEx.shouldInterceptRequest: Copying " + sourceFileName + " to " + destFileName);
				copyFile(sourceFileName, destFileName);
			}
			else
			{
				if (Global.LOG_MODE) Global.log("WebViewClientEx.shouldInterceptRequest: No copy needed");
			}
		}
	...
	{
		return (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && android.os.Build.VERSION.SDK_INT < 16 /*Build.VERSION_CODES.JELLYBEAN*/ && url.startsWith(cacheRootUrl)  && ((mDebug == false)?(getFileSize(getPathFromUrl(url)) == 0):true));
	}
	...
	private String getSourceFileNameFromCacheUrl(String url)
	{
		if (Global.LOG_MODE) Global.log("WebViewClientEx.getSourceFileNameFromCacheUrl: url is " + url);
		String sourceUrl = url.replaceFirst("file:///data/data/" + mContext.getPackageName() +"/", "");
		if (Global.LOG_MODE) Global.log("WebViewClientEx.getSourceFileNameFromCacheUrl: sourceUrl is " + sourceUrl);
		String sourceFileName = getPathFromUrl(sourceUrl);
		if (Global.LOG_MODE) Global.log("WebViewClientEx.getSourceFileNameFromCacheUrl: Returning sourceFileName " + sourceFileName);
